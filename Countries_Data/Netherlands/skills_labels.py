...
# ==== ΡΥΘΜΙΣΕ ΤΑ ====
INPUT_CSV       = r"C:\Users\Desktop PC\Desktop\ΔΙΠΛΩΜΑΤΙΚΗ\linkedin\Netherlands\UPDATED\Netherlands_with_skills.csv"  # CSV με στήλη URIs
URI_COLUMN_NAME = "skills"                                                                             # όνομα στήλης με URIs
SKILLS_MAP_CSV  = r"C:\Users\Desktop PC\Desktop\ΔΙΠΛΩΜΑΤΙΚΗ\linkedin\skills_en.csv"                              # skills_en.csv (conceptUri, preferredLabel)
OUTPUT_CSV      = None  # άστο None για αυτόματο όνομα (π.χ. *_with_names.csv)
ENCODING        = "utf-8"  # αν δεις θέμα, δοκίμασε "utf-8-sig"
# =====================
...
def parse_cell(cell):
    ...
    if not s:
        return []
    # λίστα σε string -> προσπάθησε JSON, μετά literal_eval
...
def map_uris_to_labels(uri_list):
    ...
    for u in uri_list:
        ...
        if u.startswith("http://") or u.startswith("https://"):
            ...
            if lbl is None:
                missing += 1
                names.append(u)  # fallback: κράτα το URI
            else:
                names.append(lbl)
        else:
            names.append(u)
